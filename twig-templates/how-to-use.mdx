...
<Note>
  **Dynamic Template Names:** You can even use Twig tokens *within* the template name itself to dynamically select which template to insert. For example, if you have templates named `offer-gold`, `offer-silver`, etc., you could use:
  ```twig
  {twigtemplate=offer-{{ contact.membership_tier|default('standard') }}}
  ```
  This would insert the template corresponding to the contact's `membership_tier` field.
</Note>
...
Alternatively, using the dynamic template name feature mentioned above, if your templates were named `offer-gold`, `offer-silver`, and `offer-standard`, you could simplify the above to:
...
This approach makes your main template much tidier and easier to manage. If you need to update an offer, you only edit the specific saved template (e.g., `Gold Member Offer` or `offer-gold`), and the change automatically applies everywhere it's inserted. Just ensure the template names you reference (whether static or dynamically generated) exactly match those saved in your Mautic Twig Templates list.
