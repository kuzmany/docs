...
```twig
{# Find the first company marked as primary, provide empty array as default if companies is not set #}
{% set primary_company = contact.companies|default([])|filter(c => c.is_primary == '1')|first %}

{# Display company info if found, otherwise show a message #}
{% if primary_company %}
    <p>Primary Company: {{ primary_company.companyname }}</p>
    <p>Custom Field: {{ primary_company.companycustomfield }}</p>
{% else %}
    <p>No primary company associated.</p>
{% endif %}
```
...
Use email tokens when sending emails via the API by passing them in the payload. See the [API Documentation](/twig-templates/api) for more details on managing templates via API.
