...
*   **Segments:** Access the segments (lists) the contact belongs to as an array of objects. Each object has properties like `id` and `name`.
    ...
    {% endfor %}
    ```
*   **Companies:** Access the companies associated with the contact as an array of objects. Use filters to find the primary company.
    ```twig
    {# Find the first company marked as primary #}
    {% set primary_company = contact.companies|default([])|filter(c => c.is_primary == '1')|first %}
    {% if primary_company %}
        <p>Company: {{ primary_company.companyname }}</p>
        <p>Industry: {{ primary_company.companyindustry }}</p> {# Example custom field #}
    {% endif %}
    ```
